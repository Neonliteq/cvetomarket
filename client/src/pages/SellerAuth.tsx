...
import { z } from "zod";
import { Store, ArrowLeft, Key, Copy, CheckCircle2 } from "lucide-react";
...
export default function SellerAuth() {
  ...
  const registerForm = useForm<z.infer<typeof registerSchema>>({
    ...
    defaultValues: {
      ...
      legalType: "", inn: "", ogrn: "", legalName: "", legalAddress: "",
      description: "", address: "", cityId: "",
    },
  });
  ...
  const onRegister = async (data: z.infer<typeof registerSchema>) => {
    try {
      ...
      await register({ ...payload, role: "shop" });
      toast({ title: "Добро пожаловать!", description: "Магазин создан, ожидайте подтверждения" });
  ...
  const onReset = async (data: z.infer<typeof resetSchema>) => {
    try {
      ...
      const json = await res.json();
      if (!res.ok) { toast({ title: json.error || "Ошибка", variant: "destructive" }); return; }
    ...
    <div className="min-h-[calc(100vh-3.5rem)] flex items-center justify-center p-4 bg-gradient-to-br from-primary/5 to-transparent">
      <div className="w-full max-w-lg">
        <div className="text-center mb-8">
          ...
          <p className="text-muted-foreground text-sm mt-1">ЦветоМаркет — платформа для цветочных магазинов</p>
        </div>
          ...
          <CardContent className="pt-6">
            ...
            {view === "forgot" && (
              <div className="space-y-5">
                <button onClick={() => setView("tabs")} className="flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground">
                  <ArrowLeft className="w-4 h-4" /> Назад
                </button>
                <div>
                  <h2 className="text-lg font-semibold">Восстановление пароля</h2>
                  <p className="text-sm text-muted-foreground mt-1">Ссылка для сброса пароля будет отправлена на почту и/или в Telegram</p>
                </div>
                <Form {...forgotForm}>
              ...
              <div className="space-y-5">
                {forgotResult?.emailSent ? (
                  <div className="text-center space-y-3">
                    ...
                    <p className="text-sm text-muted-foreground">Проверьте почту. Ссылка действительна 1 час.</p>
                  </div>
                  ...
                  <div className="text-center space-y-3">
                    ...
                    <h2 className="text-lg font-semibold">Ссылка отправлена в Telegram</h2>
                    <p className="text-sm text-muted-foreground">Ссылка действительна 1 час.</p>
                  </div>
                ) : (
                  <div className="space-y-4">
                    ...
                    {forgotResult?.resetLink && (
                      <>
                        <Button className="w-full" onClick={() => window.location.href = forgotResult.resetLink!}>
                          Сбросить пароль →
                        </Button>
                        <div className="relative bg-muted rounded-lg p-3 pr-12 break-all text-xs font-mono text-muted-foreground">
                          {forgotResult.resetLink}
                          <button onClick={handleCopy} className="absolute right-2 top-2 p-1.5 rounded-md hover:bg-background transition-colors">
                ...
                <Button variant="outline" className="w-full" onClick={() => { setView("tabs"); setTab("login"); }}>
                  Вернуться к входу
                </Button>
              </div>
            )}
                  ...
                  <form onSubmit={resetForm.handleSubmit(onReset)} className="space-y-4">
                    <FormField control={resetForm.control} name="password" render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl><Input {...field} type="password" placeholder="Минимум 6 символов" /></FormControl>
                        <FormMessage />
                      </FormItem>
                    )} />
                    <FormField control={resetForm.control} name="confirmPassword" render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl><Input {...field} type="password" placeholder="Повторите пароль" /></FormControl>
                        <FormMessage />
                      </FormItem>
                    )} />
            ...
            {view === "reset-success" && (
              <div className="text-center space-y-4">
                ...
                <Button className="w-full" onClick={() => { setView("tabs"); setTab("login"); navigate("/seller-auth"); }}>
                  Войти
                </Button>
              </div>
            )}
            ...
            {view === "tabs" && (
              <Tabs value={tab} onValueChange={setTab}>
                <TabsList className="w-full mb-6">
                  ...
                  <TabsTrigger value="register" className="flex-1">Открыть магазин</TabsTrigger>
                </TabsList>
                  ...
                  <Form {...loginForm}>
                    <form onSubmit={loginForm.handleSubmit(onLogin)} className="space-y-4">
                      <FormField control={loginForm.control} name="email" render={({ field }) => (
                        <FormItem>
                          <FormLabel>Email</FormLabel>
                          <FormControl><Input {...field} type="email" placeholder="you@example.com" data-testid="seller-input-email" /></FormControl>
                          <FormMessage />
                        </FormItem>
                      )} />
                      <FormField control={loginForm.control} name="password" render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl><Input {...field} type="password" placeholder="••••••" data-testid="seller-input-password" /></FormControl>
                          <FormMessage />
                        </FormItem>
                      )} />
                      <Button type="submit" className="w-full" disabled={loginForm.formState.isSubmitting} data-testid="seller-button-login">
                        {loginForm.formState.isSubmitting ? "Входим..." : "Войти в кабинет"}
                      </Button>
                    </form>
                  </Form>
                  <div className="mt-4 flex flex-col items-center gap-2">
                    <button onClick={() => setView("forgot")} className="text-sm text-muted-foreground hover:text-foreground">
                      Забыли пароль?
                    </button>
                    ...
                    <form onSubmit={registerForm.handleSubmit(onRegister)} className="space-y-4">
                      <FormField control={registerForm.control} name="name" render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl><Input {...field} placeholder='Цветочный магазин "Весна"' data-testid="seller-input-name" /></FormControl>
                          <FormMessage />
                        </FormItem>
                      )} />
                      <FormField control={registerForm.control} name="email" render={({ field }) => (
                        <FormItem>
                          <FormLabel>Email</FormLabel>
                          <FormControl><Input {...field} type="email" placeholder="you@example.com" data-testid="seller-input-reg-email" /></FormControl>
                          <FormMessage />
                        </FormItem>
                      )} />
                      <FormField control={registerForm.control} name="phone" render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl><Input {...field} placeholder="+7 (999) 000-00-00" data-testid="seller-input-phone" /></FormControl>
                          <FormMessage />
                        </FormItem>
                      )} />
                      <FormField control={registerForm.control} name="password" render={({ field }) => (
                        <FormItem>
                          <FormLabel>Пароль</FormLabel>
                          <FormControl><Input {...field} type="password" placeholder="Минимум 6 символов" data-testid="seller-input-password-reg" /></FormControl>
                          <FormMessage />
                        </FormItem>
                      )} />
                      <FormField control={registerForm.control} name="confirmPassword" render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl><Input {...field} type="password" placeholder="Повторите пароль" data-testid="seller-input-confirm-password" /></FormControl>
                          <FormMessage />
                        </FormItem>
                      )} />
                      ...
                      <FormField control={registerForm.control} name="legalType" render={({ field }) => (
                        <FormItem>
                          ...
                          <Select value={field.value || ""} onValueChange={field.onChange}>
                            <FormControl><SelectTrigger data-testid="seller-select-legal-type"><SelectValue placeholder="Выберите форму" /></SelectTrigger></FormControl>
                            <SelectContent>
                      ...
                      <FormField control={registerForm.control} name="legalName" render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl><Input {...field} placeholder='ИП Иванов И.И. / ООО "Цветы"' data-testid="seller-input-legal-name" /></FormControl>
                          <FormMessage />
                        </FormItem>
                      )} />
                      <div className="grid grid-cols-2 gap-3">
                        <FormField control={registerForm.control} name="inn" render={({ field }) => (
                          <FormItem>
                            ...
                            <FormControl><Input {...field} placeholder="1234567890" data-testid="seller-input-inn" /></FormControl>
                            <FormMessage />
                          </FormItem>
                        )} />
                        <FormField control={registerForm.control} name="ogrn" render={({ field }) => (
                          <FormItem>
                            ...
                            <FormControl><Input {...field} placeholder="1234567890123" data-testid="seller-input-ogrn" /></FormControl>
                            <FormMessage />
                          </FormItem>
                        )} />
                      </div>
                      <FormField control={registerForm.control} name="legalAddress" render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl><Input {...field} placeholder="г. Москва, ул. Примерная, д. 1" data-testid="seller-input-legal-address" /></FormControl>
                          <FormMessage />
                        </FormItem>
                      )} />
                      ...
                      <FormField control={registerForm.control} name="description" render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl><Textarea {...field} placeholder="Расскажите о вашем магазине..." data-testid="seller-input-description" /></FormControl>
                          <FormMessage />
                        </FormItem>
                      )} />
                      <FormField control={registerForm.control} name="cityId" render={({ field }) => (
                        <FormItem>
                          ...
                          <Select value={field.value || ""} onValueChange={field.onChange}>
                            <FormControl><SelectTrigger data-testid="seller-select-city"><SelectValue placeholder="Выберите город" /></SelectTrigger></FormControl>
                            <SelectContent>
                      ...
                      <FormField control={registerForm.control} name="address" render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl><Input {...field} placeholder="Фактический адрес точки" data-testid="seller-input-address" /></FormControl>
                          <FormMessage />
                        </FormItem>
                      )} />

                      <Button type="submit" className="w-full" disabled={registerForm.formState.isSubmitting} data-testid="seller-button-register">
                        {registerForm.formState.isSubmitting ? "Создаём магазин..." : "Зарегистрировать магазин"}
                      </Button>
                    </form>
                  </Form>
                </TabsContent>
